...
Full and Requirements modes verify state data against authoritative sources, save a report at `research/MM-YYYY/periodic-research-MM-DD-YYYY-HHMM.md`, and present findings one change at a time for approval. Every such report ends with a **Verification gaps** section naming the states whose Tier 1/Tier 2 source could not be read directly; those findings are provisional, not confirmed.

#### Fetch fallback ladder

Many official state election sites sit behind CDN bot management that 403s the `WebFetch` fetcher specifically — the page is reachable, the tool's user agent is not. The 2026-07-31 requirements run lost 16 of 51 Tier 1 sources this way. The skill's **Fetch fallback ladder** section handles it: WebFetch → `curl` with a descriptive project user agent (`Bash(curl *)` is in the skill's `allowed-tools`, and `Bash` is in the routines' `allowed_tools`, so this works unattended too) → Wayback snapshot → alternate official `.gov` host → Tier 3. Spoofing a browser user agent is explicitly out of bounds; raw curl output needs tag-stripping, and an empty body means a failed fetch (likely JS-rendered), not an empty page.

Four states are **robots.txt disallows**, in two categories. Three are named `ClaudeBot` opt-outs (`Disallow: /`): `elections.il.gov` (IL), `sos.wa.gov` (WA), and `sos.iowa.gov` (IA, added 2026-08-27). The fourth, `www.sec.state.ma.us` (MA), names no bot at all but carries a blanket `User-agent: *` with `Disallow: /elections` — not a Claude-specific opt-out, yet binding on every crawler all the same. All four are deliberate publisher decisions, so the skill treats them as Tier 3-only and does not route around them. A newly-403ing host gets its `robots.txt` checked before escalating to Wayback, and joins the list in whichever category fits; note that a `robots.txt` which itself returns a WAF challenge or 403 is bot mitigation, **not** an opt-out. Crucially, a disallow binds the *host*, not the state — a different official host often supplies a real Tier 1 read (on 2026-08-27, `sos.wa.gov` was left untouched while `app.leg.wa.gov`, which serves no `robots.txt`, corrected Washington's SB 6035 entry from Tier 3 to Tier 1), so state legislature bill systems should be tried before settling for Tier 3. News modes gather up to 5 recent election news items per state from reputable sources, append a run to `_data/stateNews.json`, add "Recent News" change-log entries to `_data/states.json`, and overwrite `_data/latestRunSummary.json` with a short prose summary of the run's notable themes. News capture follows the "National stories" rule (record a national/federal event for at most one representative state — see News duplication guardrails above); violating it will fail the build. Interactive modes stop after committing and present merge/discard options; autonomous mode does the merge inline.
